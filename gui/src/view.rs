    ...
    responsive(move |size| {
        ...
        for &ix in &app.filtered {
            ...
            let cover = match album.cover.as_ref().and_then(|c| app.thumbnails.get(&c.id)) {
                Some(handle) => Cover::Art(handle.clone()),
                None => Cover::Tile(album.accent.map(crate::model::color)),
            };
        ...
        // and `hover` shows them only if it really is, so an arrow-key selection shows none.
        if let Some(cell) = app.selected {
