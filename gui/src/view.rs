...
use crate::model::{App, ScanState, View, album_runs, glow_seed, run_of};
...
pub fn view(app: &App) -> Element<'_, Msg> {
    ...
    let mut layers: Vec<Element<'_, Msg>> = vec![background::background(app.glow, glow_seed(app)).into(), body, tabs.into()];
...
fn now_playing_view(app: &App) -> Element<'_, Msg> {
    ...
    let flow = cover_flow(covers, app.anim_pos, app.glow, glow_seed(app), PLAYER_BAR_HEIGHT, Msg::CoverClicked);
