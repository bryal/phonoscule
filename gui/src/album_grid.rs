...
/// What fills a card's cover square.
pub enum Cover {
    ...
    Art(image::Handle),
    /// No artwork, whether it has yet to load or the album has none: a tile in the album's accent
    /// colour with the title over it, so a fresh launch shows the library as a colour mosaic that
    /// sharpens into artwork. The accent comes from the index, long before any pixels do.
    Tile(Option<Color>),
}
            ...
            renderer.with_translation(Vector::new(0.0, -offset), |renderer| {
                for ix in shown {
                    ...
                    match &card.cover {
                        ...
                        Cover::Tile(accent) => {
                            // Dimmed, so the title over it stays readable on any accent.
