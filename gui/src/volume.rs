//! Per-application volume, delegated to the OS mixer: our own PulseAudio sink inputs (PipeWire
//! speaks the same protocol via pipewire-pulse), never the samples we decode. The server also
//! remembers the volume across restarts (stream-restore), so nothing is persisted here.
//!
//! A dedicated thread owns the libpulse connection: it finds the application's sink inputs (by
//! process id -- the playback stream comes from `pulse-simple`, which doesn't expose its stream),
//! subscribes to their changes, and applies [`VolumeControl::set`] commands. Volume readings --
//! the initial one, and external changes from any mixer UI -- stream out as plain factors
//! (1.0 = 100%), so the GUI can mirror the mixer without polling.
...
/// amplification above normal, useful for quiet recordings.
...
impl VolumeControl {
    ...
    pub fn set(&self, volume: f32) {
        // Sends only fail when the worker is gone (no PulseAudio); the GUI shows no bar then.
        let _ = self.cmd.send(volume.clamp(0.0, MAX_VOLUME));
    }
}

/// Spawns the mixer thread and returns its control handle. When PulseAudio is unreachable the
/// thread ends; no readings ever arrive and set commands go nowhere.
pub fn start() -> VolumeControl {
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            if let Err(e) = serve(cmd_rx, event_tx) {
